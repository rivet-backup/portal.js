...
export default class SearchPanel extends LitElement {
	...
	constructor() {
		...
		this.searchDebounce = new Debounce({
			...
			cb: async query => {
				...
				if ((this.options.filter.groups && !this.activeCommand) || this.activeCommand == 'gr') {
	...
	selectCommand(index: number, refocus: boolean = false) {
		// Set active command
	...
	async parseCommand(command: string) {
		...
		if (this.inputText.trim().length) {
			if (prefixMatch) {
				...
				query = command.slice(prefixMatch.length).trim();
	...
	render() {
		let singleFilter = this.options
			? ((this.options.filter.identities != this.options.filter.games) != this.options.filter.chats) !=
			  this.options.filter.groups
			: false;
	...
	renderCommand(command: Command, index: number) {
		let commandAvailable = this.options
			? this.options.filter.identities == command.identities ||
			  this.options.filter.games == command.games ||
	...
	renderItem(item: OutputItem, index: number) {
		...
		if (isIdentity(item)) {
			return html`<a
				class=${classes}
				.href=${this.options.selectionCb ? null : routes.identity.build(identityRouteData(item))}
				@click=${this.options.selectionCb ? this.options.selectionCb(item) : null}
			...
			return html`<a
				class=${classes}
				.href=${this.options.selectionCb ? null : routes.group.build(groupRouteData(item))}
				@click=${this.options.selectionCb ? this.options.selectionCb(item) : null}
				@pointerenter=${this.onPointerEnterItem.bind(this, index)}
			>
