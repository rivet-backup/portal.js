import { LitElement, html, customElement, query, css, property, PropertyValues } from 'lit-element';
...
import { MainMenuItem, MenuItem } from '../sidebar/main-sidebar';
...
export default class UIRouter extends LitElement {
	...
	onPopstate(event: PopStateEvent) {
		...
		// Weird outlier, when 2 or more pages are in the forwardsHistory array and the current page is home,
		// disable forwards navigation manually (no forwards navigation can occur from the home page)
		else if (this.homePage.new && this.forwardsHistory[2] == this.fullPath) {
			...
			// this.navigate(this.homePage.src, {
			// 	pageState: this.homePage.state,
			// 	previousPage: this.homePage
			// });
		}
	...
	onMobile() {
		let mobileOnly = this.newestPage ? this.newestPage.renderResult.mobileRedirect : this.homePage.new;
		...
		if (!global.isMobile && mobileOnly) {
			let mobileRedirect = this.newestPage
				...
				: this.homePage.renderResult.mobileRedirect;
		...
		if (this.startTouch) {
			...
			if (!this.swipeDirection) {
				...
				if (
					...
					if (normX > 0.5 && normY < 0.5 && this.homePage.old && this.touchDifferenceX > 0) {
	...
	onClick(event: MouseEvent) {
		...
		for (let node of event.composedPath()) {
			...
			if (node.tagName.toLowerCase() === 'a' && !!node.attributes.getNamedItem('href')) {
	...
	navigate(url: string, opts?: NavigateOpts) {
		...
		if (opts.previousPage == null && url == this.homePage.src) {
			opts.previousPage = this.homePage;
		}
		// If navigating to a new page from the home page, clear history
		else if (this.homePage.new) {
			this.homePage.new = false;
			this.homePage.old = true;
		...
		// Update mobile home page
		if (global.isMobile && this.home && renderResult.menuItem)
			this.home.activeMenu = renderResult.menuItem;
			...
			for (let i = this.history.length - 1; i >= 0; i--) {
				...
				if (!page.back) {
					...
					if (page.new) {
						...
						// The home page isn't actually navigated "back" to since its page is permanently
	...
	canGoBack(searchSize: number = 0): boolean {
	...
	render() {
		...
		// Add the home page to the page list
		if (global.isMobile && (this.homePage.old || this.homePage.new)) {
			pageList.unshift(this.homePage);
		}
			...
			return html` <div id="base" class=${baseClasses}>
				${repeat(
					...
					(p, i) => {
						...
						if (p == this.homePage && this.canGoBack(backedPages)) return null;
