...
export default class GamePage extends LitElement {
	...
	@property({ type: String })
	leaderboardType: string = 'identities';
	...
	async toggleStar(active: boolean) {
		alert('UNIMPLEMENTED');

		// // Don't do anything if no change
		// if (active == this.profile.isStarred) return;

		// // Update identity data
		// this.profile.isStarred = active;
		// this.profile.starCount += active ? 1 : -1;
		// this.requestUpdate("profile");

		// // Commit changes
		// await GameEndpoints.setStarred.execute({ gameId: this.profile.game.id, active });
	}
					...
					<div id="actions-right">
						...
						<!-- ${this.profile
							? html` <stylized-button
									id="star"
									icon=${this.profile.isStarred ? 'solid/star' : 'regular/star'}
									large
									color="white"
									text="${this.profile.isStarred ? '#F7AE04' : 'black'}"
									.trigger=${this.toggleStar.bind(this, !this.profile.isStarred)}
							  >
									${numbro(this.profile.starCount).format('0,0')}
							  </stylized-button>`
							: null} -->
						<stylized-button
							id="play"
						...
						${repeat(
							...
							(data, i) => {
								...
								let classes = classMap({
									'is-self': id == global.currentIdentity.id
								});
