...
export default class PerfWaterfall extends LitElement {
	...
	render() {
		this.perfLists.sort((a, b) => a.ts - b.ts); // Sort by timestamp
		...
		let finishTs = Math.max(
			selectedList.spans.length ? selectedList.spans[selectedList.spans.length - 1].finishTs : 0,
			selectedList.marks.length ? selectedList.marks[selectedList.marks.length - 1].ts : 0
		);
		// Get first timestamps
		let startTs = Math.min(
			selectedList.spans.length ? selectedList.spans[0].startTs : 0,
			selectedList.marks.length ? selectedList.marks[0].ts : 0
		);
					...
					<div class="items">
						${repeat(
							...
							b => {
								...
								let duration = (b.finishTs != null ? b.finishTs : finishTs) - b.startTs;
								let width = (100 * duration) / totalDuration;
								let blockStyle = styleMap({
									'margin-left': `${(100 * (b.startTs - startTs)) / totalDuration}%`,
									width: `${width}%`
								});
								...
								// Format start ts
								let spanStartTs = b.startTs / TIME_SCALE;
								...
								if (b.finishTs != null) {
									let spanFinishTs = b.finishTs / TIME_SCALE;
									...
									${repeat(
										...
										m => {
											...
											let markStyle = styleMap({
												left: `${(100 * (m.ts - startTs)) / totalDuration}%`
											});
											...
											// Formatted timestamp
											let tsFormatted: string | number = m.ts / TIME_SCALE;
											tsFormatted =
												tsFormatted < 1
